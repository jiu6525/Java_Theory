	...
	public static void main(String[] args) {
		...
		do {//전체 반복
			...
			for(int i=1; i<=cnt ; i++) {// 게임수 만큼 반복실행
				//난수생성(7개)
				int lotto[] = new int[7];// 번호6개와 보너스를 담을 배열
				for(int j=0; j<lotto.length; j++) {// j->0(12), 1(3)
					lotto[j] = random.nextInt(45) + 1; // 1~45  =  45-1+1
					//중복검사 : 이전에 만든 값중 같은 값이 존재하는 지 찾는 후 중복되면 지금 만든값이 버려야 한다.
					for(int k=0; k<j; k++ ) {
						if(lotto[k] == lotto[j]) {
							j--;//0 
							break;
				...
				//7개의 난수 생성이 된경우
				int lottoNum[] = Arrays.copyOfRange(lotto, 0, 6);//  [43,2,53,43]
